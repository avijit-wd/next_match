...
export async function signInUser(
  ...
  try {
    ...
    if (!existingUser.emailVerified) {
      ...
      return {
        status: "error",
        error: "Please verify your email address for loging in",
      };
    }
...
export async function registerUser(
  ...
  try {
    // Added validation in server as well.
    ...
    // We can avoid await while we return query directly from server actions
...
export async function resetPassword(
  ...
  try {
    ...
    return {
      status: "success",
      data: "Password updated successfully.Please try loging in",
    };
  } catch (error) {
    console.log(error);
    return { status: "error", error: "Something went wrong" };
